...
@click.option('-of', '--outputfoldername', type=click.STRING, default='fit_images',
...
@click.option('-f', '--outputformat', type=click.STRING, default="ENVI",
              help="The GDAL driver code for the output format desired. Default is ENVI format.")
...
@click.option('-cs', '--cliptoshape', type=click.Path(exists=True, file_okay=True, dir_okay=False, readable=True,
                                                resolve_path=True),
              help="Path to a shapefile to clip the raster. If omitted, entire raster extent will be processed.")
@click.option('-ce', 'cliptopixelextent', nargs=4, type=int,
...
def find_fit(vi, signaturedirectory, image, outputdir, outputfoldername, startdoy, doyinterval, temporalshift, threshold,
             ndvalue, subset, outputformat, meantype, numberofprocesses):
    """
    ...
    phenological_classificaion(image, outputdir, outputfoldername, signatures, startdoy, doyinterval, temporalshift,
                               threshold=threshold, ndvalue=ndvalue, subset=subset, gdaldrivercode=outputformat,
                               meantype=meantype, workers=numberofprocesses)
