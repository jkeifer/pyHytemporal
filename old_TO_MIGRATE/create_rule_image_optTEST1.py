...
def interpolate_refs(refdict):

    interpdict = {}

    for key, val in refdict.items():
        x1, y1 = get_sort_dates_values(val)
        interpolatedcurve = interpolate.splrep(x1, y1)
        interpdict[key] = interpolatedcurve
...
try:
    ...
    interpolatedCurves = interpolate_refs(refs)
    ...
    for row in range(0, rows):
        for col in range(0, cols):
            ...
            count = 1
            fit = {}
            for key, val in interpolatedCurves.items():
